...
@section("add")
    ...
    <form action="{{url("admins/teacher/my-course/course/$offeredCourse->id")}}" enctype="multipart/form-data"
          method="post">
        @csrf
        <div class="form-group">
    ...
    <a href="{{url("admins/teacher/my-course/course/$offeredCourse->id/create")}}">
...
@section("tr1")
    <tr>
        ...
        <th>CT-01 OBTAINED MARK</th>
        <th>CT-02 OBTAINED MARK</th>
        <th>CT-03 OBTAINED MARK</th>
...
@section("tr2")
    @foreach($results as $result)
        <tr>
            ...
            <td>{{$result->ct1_obtained}}</td>
            <td>{{$result->ct2_obtained}}</td>
            <td>{{$result->ct3_obtained}}</td>
            <td>{{$result->mid_obtained}}</td>
            <td>{{$result->final_obtained}}</td>
            <td>{{$result->letter_grade}}</td>
            <td>
                <a href="{{url("admins/teacher/my-course/course/$result->id/edit")}}">
            ...
            <td>
                <form action="{{url("admins/teacher/my-course/course/$result->id")}}" method="post"
